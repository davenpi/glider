...
def gamma(u: float, v: float, w: float, beta: float) -> float:
    ...
    """
    gamma = -2 * CT * a(beta) * u * v / speed(u, v) + 2 * CR * (a(beta) ** 2) * w
    return gamma
...
def M(w: float, beta: float) -> float:
    ...
    """
    L = a(beta)
    M = (np.pi * rho_f * a(beta) ** 4) * (mu * V(beta) / L + nu * np.abs(w)) * w
    return M


def V(beta: float) -> float:
    """
    ...
    """
    V = np.sqrt(np.abs(rho_s / rho_f - 1) * g * b(beta))
    return V
